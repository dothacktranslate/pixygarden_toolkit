...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        description="Patch/repack standalone STAGE HELP.FAT text from a PixyGarden Planet text workbook.",
    ...
    ap.add_argument("--source-help-fat", required=True, help="Source/current DATA/STAGE/HELP.FAT")
    ap.add_argument("--out-help-fat", required=True, help="Patched HELP.FAT output path")
    ...
    ap.add_argument("--root-path", default="HELP.FAT", help="Archive root path label. Default HELP.FAT")
    ...
    ap.add_argument("--pad-to-original", action="store_true", help="If rebuilt HELP.FAT is smaller than original, pad back to original length with pad byte")
    ap.add_argument("--must-fit-original", action="store_true", help="Fail if rebuilt HELP.FAT is larger than the original file")
    ...
    source = Path(args.source_help_fat)
    old_help = source.read_bytes()
    entries = recursive_fat_manifest(old_help, args.root_path)
    if args.write_manifest:
        write_manifest_csv(Path(args.write_manifest), entries)

    builds, sheet_name = load_txt_builds(args)
    rebuilt_used, rows, meta = rebuild_help_fat(old_help, args.root_path, builds, args)

    warnings: list[str] = []
    if meta["missing_build_keys"]:
        warnings.append(f"{len(meta['missing_build_keys'])} workbook rows did not match entries in {args.root_path}")

    if args.must_fit_original and len(rebuilt_used) > len(old_help):
        raise SystemExit(f"Rebuilt HELP.FAT uses 0x{len(rebuilt_used):X}, exceeding original file size 0x{len(old_help):X}")
    rebuilt_write = rebuilt_used
    if args.pad_to_original and len(rebuilt_write) < len(old_help):
        rebuilt_write += bytes([args.pad_byte & 0xFF]) * (len(old_help) - len(rebuilt_write))

    for r in rows:
        r["root_old_size"] = f"0x{len(old_help):X}"
        r["root_new_used_size"] = f"0x{len(rebuilt_used):X}"
        r["root_written_size"] = f"0x{len(rebuilt_write):X}"
        r["root_delta"] = len(rebuilt_write) - len(old_help)
    write_report(Path(args.report), rows, ["root_old_size", "root_new_used_size", "root_written_size", "root_delta"])

    verify_errors = verify_help_texts(rebuilt_used, builds, args.root_path, parse_byte(args.terminator, 0x00) or 0x00)
    if verify_errors:
    ...
    if not args.dry_run:
        out = Path(args.out_help_fat)
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_bytes(rebuilt_write)

    print("PixyGarden STAGE HELP.FAT text patcher")
    print("---------------------------------------")
    print(f"Workbook:             {args.xlsx}")
    print(f"Sheet:                {sheet_name}")
    print(f"Rows loaded:          {len(builds)}")
    print(f"Source HELP.FAT:      {args.source_help_fat}")
    print(f"Old HELP.FAT size:    0x{len(old_help):X}")
    print(f"Rebuilt used size:    0x{len(rebuilt_used):X}")
    print(f"Written size:         0x{len(rebuilt_write):X}")
    print(f"Delta vs original:    {len(rebuilt_write) - len(old_help)}")
    ...
    if args.dry_run:
        print("Dry run: no binary output written.")
    else:
        print(f"Patched HELP.FAT:     {args.out_help_fat}")
        print(f"Output SHA-256:       {hashlib.sha256(Path(args.out_help_fat).read_bytes()).hexdigest()}")
    if warnings:
        print("Warnings:")
        for w in warnings:
            print(f"  - {w}")
